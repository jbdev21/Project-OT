        ...
        <div class="x_panel">
            <div class="x_title">
          	...
          	<div class="row">
                  ...
                  <input type="text" class="form-control" readonly="" value="{{$comment->user->username}}">
          			   <input type="hidden"  name="user_id" value="{{$comment->user->id}}">
          			</p>
          			<p>
          				<label>@lang('label.month')</label>
          				<input readonly class="form-control" name="month" type="month" required value="{{$comment->month}}">
          			</p>
                    @if($comment->admin_id != Auth::user()->id)
                       <p>
                        <label for="">@lang('label.teacher')</label>
                        <input type="text" class="form-control" readonly value="{{ $comment->admin->name }}">
                       </p>
                       <br>
                    @endif
          			<p>
                        ...
                        {{csrf_field()}}
                        <textarea  name="message" id="my-editor" rows="8" class="form-control">{!! $comment->message!!}</textarea>
                        <br>
                        <button type="submit" class="btn btn-success"><i class="fa fa-save"></i> @lang('button.save_changes')</button>
                        <a type="submit" href="{{ route('admin.comment.show', ['id' => $comment->user_id, 'month' => $comment->month]) }}" class="btn btn-warning"><i class="fa fa-reply-all"></i> @lang('button.back')</a>
          			</p>
          		</div>
                </form>
          	</div>
