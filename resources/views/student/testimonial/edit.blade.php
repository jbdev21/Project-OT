...
@if(is_desktop())
	<div class="box padding-sm">
		...
		<div class="row">
			<form action="{{ route('dashboard.testimonial.update', $testimonial->id) }}" method="post">
				{{csrf_field()}}
				{{method_field('PUT')}}
				<div class="col-sm-8 col-sm-offset-2">
          <p>
            <label>@lang('label.teacher')</label>
            <select class="form-control" name="admin_id">
              @foreach($teachers as $teacher)
                <option @if($testimonial->admin_id == $teacher->id) selected @endif  value="{{$teacher->id}}">{{$teacher->name}}</option>
              @endforeach
            </select>
          </p>
					<textarea id="my-editor" name="message">{!! $testimonial->message !!}</textarea>
					<br>
...
@else
  	<h6 class="mobile-title">@lang('label.testimonial')</h6>
    <div class="container">
			<form action="{{ route('dashboard.testimonial.update', $testimonial->id) }}" method="post">
				{{csrf_field()}}
				{{method_field('PUT')}}
