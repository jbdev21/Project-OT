<template>
     <li role="presentation" class="dropdown" @click="MarkAllRead">
...
export default {
    data(){
        return {
            notifications:[],
            notificationurl: baseUrl + '/admin/notifications',
            unread:0
        }
    },
    created()
    ...
    methods:{
        MarkAllRead()
        {
           axios.get(baseUrl + '/admin/api/notification/markall')
            .then( response => {
                if(response.data)
                {
                    this.getNoti();
        ...
        getNoti()
        {
            axios.get(baseUrl + '/admin/api/notification')
            .then( response => {
        ...
        getUnread()
        {
            axios.get(baseUrl + '/admin/api/unreadnotification')
            .then( response => {
