...
class Setting extends Model
{
     ...
     * @return array
     */
    public static function getValidationRules($file)
    {
        return self::getDefinedSettingFields($file)->pluck('rules', 'name')
            ->reject(function ($val) {
            return is_null($val);
        })->toArray();
    }
     ...
     * @return Collection
     */
    private static function getDefinedSettingFields($file=null)
    {
        if($file){
            return collect(config($file))->pluck('elements')->flatten(1);
        }else{
            return collect(config('settings_files'))->pluck('elements')->flatten(1);
