                                ...
                                <td>
                                    <select type='text' name="time" class="form-control">
                                        <?php $__currentLoopData = time_select(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $time): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <option value="<?php echo e(date('h:iA', strtotime($time))); ?>" <?php if(date('h:iA', strtotime($leveltest->time)) ==  date('h:iA', strtotime($time)) ): ?> selected  <?php endif; ?> > <?php echo e(date('h:iA', strtotime($time))); ?></option>
                            ...
                            <tr>
                                ...
                                <td>
                                    <select name="self_assesment" class="form-control">
                                        <option <?php if($leveltest->self_assesment == "LV1/4 Starter"): ?> selected <?php endif; ?> value="LV1/4 Starter">LV1/4 시작레벨 (알파벳 읽기 가능)</option>
                                        <option <?php if($leveltest->self_assesment == "LV2/4 Can Read Word"): ?> selected <?php endif; ?> value="LV2/4 Can Read Word">LV21/4 준초급 (단어형태 읽기 가능)  </option>
                                        <option <?php if($leveltest->self_assesment == "LV3/4 Can Speak Little"): ?> selected <?php endif; ?> value="LV3/4 Can Speak Little">LV3/4 초급(천천히 대화 가능) </option>
                                        <option <?php if($leveltest->self_assesment == "LV4/4 Can Communicate"): ?> selected <?php endif; ?> value="LV4/4 Can Communicate">LV4/4 중급이상 (일상대화 가능)  </option>
                                    </select>
                                </td>
                            </tr>
                            <tr>
                                <td><label for=""><?php echo app('translator')->getFromJson('label.book'); ?> </label></td>
                                <td>
                                    <select class="form-control" name="book_id">
                                        <option value="">- 교재 미지정 (수업후 배정 됩니다) -</option>
                                        <?php $__currentLoopData = $books; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $book): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <option value="<?php echo e($book->id); ?>" <?php if($leveltest->book_id == $book->id): ?> selected <?php endif; ?>><?php echo e($book->title); ?></option>
                                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                    </select>
                                </td>
                            </tr>
